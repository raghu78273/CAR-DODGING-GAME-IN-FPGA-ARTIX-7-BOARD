module car_dodging_game_top (
    ...
    reg btn_left_frame_pulse = 1'b0;
    reg btn_right_frame_pulse = 1'b0;
    reg btn_start_frame_pulse = 1'b0;
    reg btn_pause_frame_pulse = 1'b0;
    reg btn_restart_frame_pulse = 1'b0;
    ...
    wire [19:0] score_bcd;
    wire [19:0] player1_score_bcd;
    wire [19:0] player2_score_bcd;
    ...
    always @(posedge clk50mhz) begin
        ...
        if (frame_tick) begin
            btn_left_frame_pulse <= btn_left_pulse;
            btn_right_frame_pulse <= btn_right_pulse;
            btn_start_frame_pulse <= btn_start_pulse;
            btn_pause_frame_pulse <= btn_pause_pulse;
            btn_restart_frame_pulse <= btn_restart_pulse;
        end else begin
            if (btn_left_pulse) begin
                btn_left_frame_pulse <= 1'b1;
            end
            if (btn_right_pulse) begin
                btn_right_frame_pulse <= 1'b1;
            end
            if (btn_start_pulse) begin
                btn_start_frame_pulse <= 1'b1;
            end
            if (btn_pause_pulse) begin
                btn_pause_frame_pulse <= 1'b1;
            end
            if (btn_restart_pulse) begin
                btn_restart_frame_pulse <= 1'b1;
            end
        end
    end
    ...
    game_state #(
        ...
        .clk(clk50mhz),
        .frame_tick(frame_tick),
        .btn_left_pulse(btn_left_frame_pulse),
        .btn_right_pulse(btn_right_frame_pulse),
        .btn_start_pulse(btn_start_frame_pulse),
        .btn_pause_pulse(btn_pause_frame_pulse),
        .btn_restart_pulse(btn_restart_frame_pulse),
    ...
    bin_to_bcd5 player1_score_conv (
        .binary(player1_score),
        .bcd(player1_score_bcd)
    );

    bin_to_bcd5 player2_score_conv (
        .binary(player2_score),
        .bcd(player2_score_bcd)
    );
    ...
    game_renderer #(
        ...
        .enemy_style_bus(enemy_style_bus),
        .score_bcd(score_bcd),
        .player1_score_bcd(player1_score_bcd),
        .player2_score_bcd(player2_score_bcd),
